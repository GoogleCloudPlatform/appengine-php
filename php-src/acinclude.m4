...
dnl  AC_CHECK_SIZEOF(long int, 4)
dnl  AC_MSG_CHECKING([checking if we're at 64-bit platform])
dnl  if test "$ac_cv_sizeof_long_int" = "4" ; then
...
dnl    $2
dnl  fi
  AC_MSG_CHECKING([native client is a 32 bit system])
  $1
...
dnl  if test "$PHP_OPENSSL_DIR" = "yes" && test -x "$PKG_CONFIG" && $PKG_CONFIG --exists openssl; then
dnl    if $PKG_CONFIG --atleast-version=0.9.6 openssl; then
dnl      found_openssl=yes
dnl      OPENSSL_LIBS=`$PKG_CONFIG --libs openssl`
dnl      OPENSSL_INCS=`$PKG_CONFIG --cflags-only-I openssl`
dnl      OPENSSL_INCDIR=`$PKG_CONFIG --variable=includedir openssl`
dnl    else
dnl      AC_MSG_ERROR([OpenSSL version 0.9.6 or greater required.])
dnl    fi
dnl
dnl    if test -n "$OPENSSL_LIBS" && test -n "$OPENSSL_INCS"; then
dnl      PHP_EVAL_LIBLINE($OPENSSL_LIBS, $1)
dnl      PHP_EVAL_INCLINE($OPENSSL_INCS)
dnl    fi
dnl  fi
...
dnl      if test -r $i/$PHP_LIBDIR/libssl.a -o -r $i/$PHP_LIBDIR/libssl.$SHLIB_SUFFIX_NAME; then
dnl        OPENSSL_LIBDIR=$i/$PHP_LIBDIR
dnl      fi
dnl      test -n "$OPENSSL_INCDIR" && test -n "$OPENSSL_LIBDIR" && break
    done
...
dnl    if test -z "$OPENSSL_LIBDIR"; then
dnl      AC_MSG_ERROR([Cannot find OpenSSL's libraries])
dnl    fi
...
#include <openssl/opensslv.h>
  ...
  yes
    ...
    AC_MSG_WARN([yo yo yo])
    AC_MSG_NOTICE([$OPENSSL_INCDIR])
    PHP_ADD_INCLUDE($OPENSSL_INCDIR)
    found_openssl=yes

dnl    PHP_CHECK_LIBRARY(crypto, CRYPTO_free, [
dnl      PHP_ADD_LIBRARY(crypto,,$1)
dnl    ],[
dnl      AC_MSG_ERROR([libcrypto not found!])
dnl    ],[
dnl      -L$OPENSSL_LIBDIR
dnl    ])

dnl    old_LIBS=$LIBS
dnl    LIBS="$LIBS -lcrypto"
dnl    PHP_CHECK_LIBRARY(ssl, SSL_CTX_set_ssl_version, [
dnl      found_openssl=yes
dnl    ],[
dnl      AC_MSG_ERROR([libssl not found!])
dnl    ],[
dnl      -L$OPENSSL_LIBDIR
dnl    ])
dnl    LIBS=$old_LIBS
dnl    PHP_ADD_LIBRARY(ssl,,$1)
dnl    PHP_ADD_LIBRARY(crypto,,$1)

dnl    PHP_ADD_LIBPATH($OPENSSL_LIBDIR, $1)
  fi
...
dnl    if test -f $ICONV_DIR/$PHP_LIBDIR/lib$iconv_lib_name.a ||
dnl       test -f $ICONV_DIR/$PHP_LIBDIR/lib$iconv_lib_name.$SHLIB_SUFFIX_NAME
dnl    then
dnl      PHP_CHECK_LIBRARY($iconv_lib_name, libiconv, [
dnl        found_iconv=yes
dnl        PHP_DEFINE(HAVE_LIBICONV,1,[ext/iconv])
dnl        AC_DEFINE(HAVE_LIBICONV,1,[ ])
dnl        PHP_DEFINE([ICONV_ALIASED_LIBICONV],1,[ext/iconv])
dnl        AC_DEFINE([ICONV_ALIASED_LIBICONV],1,[iconv() is aliased to libiconv() in -liconv])
dnl      ], [
dnl        PHP_CHECK_LIBRARY($iconv_lib_name, iconv, [
dnl          found_iconv=yes
dnl        ], [], [
dnl          -L$ICONV_DIR/$PHP_LIBDIR
dnl        ])
dnl      ], [
dnl        -L$ICONV_DIR/$PHP_LIBDIR
dnl      ])
...
dnl      PHP_ADD_LIBRARY_WITH_PATH($iconv_lib_name, $ICONV_DIR/$PHP_LIBDIR, $1)
...
[
  ...
  if test -x "$ac_cv_php_xml2_config_path"; then
    ...
    if test "$LIBXML_VERSION" -ge "2006011"; then
      LIBXML_LIBS=`$XML2_CONFIG --prexif=$PHP_LIBXML_DIR --libs`
      LIBXML_INCS=`$XML2_CONFIG --prefix=$PHP_LIBXML_DIR --cflags`
dnl      PHP_EVAL_LIBLINE($LIBXML_LIBS, $1)
      PHP_EVAL_INCLINE($LIBXML_INCS)
      AC_DEFINE(HAVE_LIBXML, 1, [ ])

      dnl Check that build works with given libs
dnl      AC_CACHE_CHECK(whether libxml build works, php_cv_libxml_build_works, [
dnl        PHP_TEST_BUILD(xmlInitParser,
dnl        [
dnl          php_cv_libxml_build_works=yes
dnl        ], [
dnl          AC_MSG_RESULT(no)
dnl          AC_MSG_ERROR([build test failed.  Please check the config.log for details.])
dnl        ], [
dnl          [$]$1
dnl        ])
dnl      ])
dnl      if test "$php_cv_libxml_build_works" = "yes"; then
dnl        AC_DEFINE(HAVE_LIBXML, 1, [ ])
dnl      fi
dnl      $2
    else
