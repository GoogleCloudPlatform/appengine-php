...
dnl    PHP_CHECK_LIBRARY(jpeg,jpeg_read_header,
dnl    [
dnl      PHP_ADD_INCLUDE($GD_JPEG_DIR/include)
dnl      PHP_ADD_LIBRARY_WITH_PATH(jpeg, $GD_JPEG_DIR/$PHP_LIBDIR, GD_SHARED_LIBADD)
dnl    ],[
dnl      AC_MSG_ERROR([Problem with libjpeg.(a|so). Please check config.log for more information.])
dnl    ],[
dnl      -L$GD_JPEG_DIR/$PHP_LIBDIR
dnl    ])
  else
...
dnl    PHP_CHECK_LIBRARY(png,png_write_image,
dnl    [
dnl      PHP_ADD_INCLUDE($GD_PNG_DIR/include)
dnl      PHP_ADD_LIBRARY_WITH_PATH(z, $PHP_ZLIB_DIR/$PHP_LIBDIR, GD_SHARED_LIBADD)
dnl      PHP_ADD_LIBRARY_WITH_PATH(png, $GD_PNG_DIR/$PHP_LIBDIR, GD_SHARED_LIBADD)
dnl    ],[
dnl      AC_MSG_ERROR([Problem with libpng.(a|so) or libz.(a|so). Please check config.log for more information.])
dnl    ],[
dnl      -L$PHP_ZLIB_DIR/$PHP_LIBDIR -lz -L$GD_PNG_DIR/$PHP_LIBDIR
dnl    ])
  else
...
dnl    PHP_CHECK_LIBRARY(freetype, FT_New_Face,
dnl    [
dnl      PHP_ADD_LIBRARY_WITH_PATH(freetype, $FREETYPE2_DIR/$PHP_LIBDIR, GD_SHARED_LIBADD)
dnl      PHP_ADD_INCLUDE($FREETYPE2_DIR/include)
dnl      PHP_ADD_INCLUDE($FREETYPE2_INC_DIR)
dnl      AC_DEFINE(USE_GD_IMGSTRTTF, 1, [ ])
dnl      AC_DEFINE(HAVE_LIBFREETYPE,1,[ ])
dnl      AC_DEFINE(ENABLE_GD_TTF,1,[ ])
dnl    ],[
dnl      AC_MSG_ERROR([Problem with freetype.(a|so). Please check config.log for more information.])
dnl    ],[
dnl      -L$FREETYPE2_DIR/$PHP_LIBDIR
dnl    ])
  else
