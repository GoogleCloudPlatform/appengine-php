...
dnl  PHP_MCRYPT_CHECK_VERSION

dnl  PHP_CHECK_LIBRARY(mcrypt, mcrypt_module_open, 
dnl  [
dnl    PHP_ADD_LIBRARY(ltdl,, MCRYPT_SHARED_LIBADD)
dnl    AC_DEFINE(HAVE_LIBMCRYPT,1,[ ])
dnl  ],[
dnl    PHP_CHECK_LIBRARY(mcrypt, mcrypt_module_open,
dnl    [
dnl      AC_DEFINE(HAVE_LIBMCRYPT,1,[ ])
dnl    ],[
dnl      AC_MSG_ERROR([Sorry, I was not able to diagnose which libmcrypt version you have installed.])
dnl    ],[
dnl      -L$MCRYPT_DIR/$PHP_LIBDIR
dnl    ])
dnl  ],[
dnl    -L$MCRYPT_DIR/$PHP_LIBDIR -lltdl
dnl  ])

  AC_DEFINE(HAVE_LIBMCRYPT,1,[ ])
dnl  PHP_ADD_LIBRARY_WITH_PATH(mcrypt, $MCRYPT_DIR/$PHP_LIBDIR, MCRYPT_SHARED_LIBADD)
