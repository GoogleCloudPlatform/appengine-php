                       ...
                       TSRMLS_DC)
{
    ...
    // Note: stat mode for generating the file key does not work with VFS, so
    // we'll disable caching of all files that rely on stat mode.
    apc_debug("Will not cache file %s, as stat mode is disabled.\n" TSRMLS_CC,
              filename);
    goto cleanup;
