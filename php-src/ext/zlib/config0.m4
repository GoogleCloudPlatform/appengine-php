...
dnl  PHP_CHECK_LIBRARY(z, gzgets, [
dnl    AC_DEFINE(HAVE_ZLIB,1,[ ]) 
dnl  ],[
dnl    AC_MSG_ERROR(ZLIB extension requires gzgets in zlib)
dnl  ],[
dnl    $ac_extra
dnl  ])
  AC_DEFINE(HAVE_ZLIB,1,[ ])

dnl  PHP_ADD_LIBPATH($ZLIB_DIR/$PHP_LIBDIR, ZLIB_SHARED_LIBADD)

  PHP_ZLIB_DIR=$ZLIB_DIR
dnl  PHP_ADD_LIBRARY(z,, ZLIB_SHARED_LIBADD)
