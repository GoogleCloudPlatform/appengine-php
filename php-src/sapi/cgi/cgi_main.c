...
// A drop in syslog replacement that is linked into the native client PHP
// interpreter. See http://en.wikipedia.org/wiki/Syslog.
// Arguments:
// - priority: A syslog severity value
// - format_string: A printf style format string, unused in PHP.
// - message: The already formattted log message.
void gae_syslog(int priority,
                const char *format_string,
                const char *message TSRMLS_DC) {
	zval fname;
	zval *z_priority, *z_format_string, *z_message;
	zval **params[3];
	zval *retval_ptr;
	int result;
	...
	INIT_PZVAL(&fname);
	ZVAL_STRING(&fname, "_gae_syslog", 1);

	MAKE_STD_ZVAL(z_priority);
	ZVAL_LONG(z_priority, priority);
	params[0] = &z_priority;

	MAKE_STD_ZVAL(z_format_string);
	ZVAL_STRING(z_format_string, format_string, 1);
	params[1] = &z_format_string;

	MAKE_STD_ZVAL(z_message);
	ZVAL_STRING(z_message, message, 1);
	params[2] = &z_message;

	result = call_user_function_ex(
		CG(function_table), NULL, &fname, &retval_ptr,
		3, params, 0, NULL TSRMLS_CC);

	zval_dtor(&fname);
	zval_ptr_dtor(&z_priority);
	zval_ptr_dtor(&z_format_string);
	zval_ptr_dtor(&z_message);

	if (result == FAILURE) {
		php_error(E_WARNING, "Unexpected failure during call to syslog");
	}
