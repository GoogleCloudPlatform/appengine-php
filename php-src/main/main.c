...
PHPAPI void php_log_err(char *log_message TSRMLS_DC)
{
	...
	if (PG(error_log) != NULL) {
		...
		if (!strcmp(PG(error_log), "syslog")) {
			php_syslog(LOG_ERR, "%s", log_message TSRMLS_CC);
			PG(in_error_log) = 0;
			return;
		}
#endif
	...
	switch (type) {
		case E_CORE_ERROR:
			if(!module_initialized) {
				...
				exit(PHP_MODULE_INITIALIZATION_FAILED);
			}
...
// Note: With the read only filesystem in GAE we can enable open_basedir and
// the realpath_cache as there is no risk of the file under the cache entry
// changing.
