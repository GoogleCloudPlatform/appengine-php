...
static void read_headers_from_part(multipart_buffer *self,
                                    ...
                                    zend_llist *header TSRMLS_DC)
{
  ...
  // Read the headers from this part.
  read_headers_from_part(self, header TSRMLS_CC);
...
static const char kMessageExternal[] = "message/external-body";
static const char kBlobKey[] = "blob-key";
static const char kGSFilePrefix[] = "/gs/";
static const char kGSStreamPrefix[] = "gs://";
...
SAPI_API SAPI_POST_HANDLER_FUNC(rfc1867_post_handler) /* {{{ */
{
	...
	long count = 0;
  // Additional variables for GCS uploads
  ...
  const char* content_md5 = NULL;
	...
	while (!multipart_buffer_eof(mbuff TSRMLS_CC))
	{
		...
		// Support for uploading to GCS or Blobstore. The scotty-agent will post
		// a message that has Content-Type: message/external-body; blob-key=xxx
		// If we see this we can proceed to fill in the $_FILES array with the
		// content for the upload.
		if ((cd = php_mime_get_hdr_value(header, "Content-Type"))) {
		  // We need to take a copy of the header content as php_mime_get_hdr_value
		  // has returned the underlying pointer.
		  char* value = estrdup(cd);
		  int external_body_found = 0;
		  int blob_key_found = 0;
		  ...
		  while(*value &&
            (field = getword(mbuff->input_encoding, &value, ';' TSRMLS_CC))) {
		    while(isspace(*value)) {
		      ++value;
		    }
		    if (!strcasecmp(field, kMessageExternal)) {
          external_body_found = 1;
		    } else if (!strncasecmp(field, kBlobKey, strlen(kBlobKey))) {
          blob_key_found = 1;
		    }
		  }
      gcs_upload = (external_body_found == 1) && (blob_key_found == 1);
		}
		...
		if ((cd = php_mime_get_hdr_value(header, "Content-Disposition"))) {
			...
			// If a blob_key has been defined then we are dealing with an upload to
			// GCS or blobstore. The body of the message will be headers that we
			// can stuff into the files array.
			if (gcs_upload) {
			  if (!filename) {
			    php_error_docref(NULL TSRMLS_CC, E_ERROR,
			                     "No filenamed defined for a Cloud Storage upload.");
			    continue;
			  }
			  // Read the headers from the body of the message.
			  zend_llist_clean(&header);

			  read_headers_from_part(mbuff, &header TSRMLS_CC);

			  const char* gs_filename = php_mime_get_hdr_value(header,
			      "X-AppEngine-Cloud-Storage-Object");
			  if (gs_filename == NULL) {
			    sapi_module.sapi_error(
			        E_ERROR, "Google Cloud Storage filename missing from upload");
			    continue;
			  }

			  content_length = php_mime_get_hdr_value(header, "Content-Length");
			  if (content_length == NULL) {
			    sapi_module.sapi_error(
			        E_ERROR, "Google Cloud Storage file length missing from upload");
			    continue;
			  }

			  // TODO(slangley): Add the MD5 value to the $_FILES array.
			  content_md5 = php_mime_get_hdr_value(header, "Content-MD5");

			  int gs_prefix_len = strlen(kGSFilePrefix);
			  if (strncasecmp(gs_filename, kGSFilePrefix, gs_prefix_len)) {
			    sapi_module.sapi_error(
			        E_ERROR, "Google Cloud Storage prefix is incorrect.");
			    continue;
}

// scotty_agent sends us the GCS file in /gs/bucket/object - In PHP
// we use gcs://bucket/object so we need to change it.
char* gcs_stream_name = emalloc(strlen(gs_filename) +
                        strlen(kGSStreamPrefix));
strcpy(gcs_stream_name, kGSStreamPrefix);
strcat(gcs_stream_name, gs_filename + gs_prefix_len);
temp_filename = gcs_stream_name;
		}
      ...
      if (!gcs_upload) {
        if (!PG(file_uploads)) {
          ...
          sapi_module.sapi_error(E_WARNING, "Maximum number of allowable file uploads has been exceeded");
			...
			fd = -1;
			// We've set the name of temp_filename to the GCS file above, if this is
			// a GCS upload.
			if (!gcs_upload) {
	      temp_filename = NULL;
			}
      ...
      if (!gcs_upload) {
        if (!skip_upload && php_rfc1867_callback != NULL) {
          ...
          if (php_rfc1867_callback(MULTIPART_EVENT_FILE_START, &event_file_start, &event_extra_data TSRMLS_CC) == FAILURE) {
			...
			while (!gcs_upload && !cancel_upload && (blen > 0))
			{
				if (php_rfc1867_callback != NULL) {
			...
			if (!gcs_upload && !cancel_upload && !end) {
#if DEBUG_FILE_UPLOAD
...
#endif
			if (!gcs_upload && php_rfc1867_callback != NULL) {
          ...
          if (!gcs_upload) {
            ...
            file_size.value.lval = atol(content_length);
          }
					file_size.type = IS_LONG;
				}
