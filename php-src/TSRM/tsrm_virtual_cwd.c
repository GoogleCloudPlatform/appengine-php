...
#undef LINK_MAX
#define LINK_MAX 32
// Notes (slangley): This function is called recursively to try and resolve a
// path to the actual file on disk. In the AppServer environment this means
// that it attempts to stat directories that are outside of the application
// which results in spamming the logs with fd_proxy error messages, like
// W1016 16:40:58.488744 24829 fd_proxy.cc:125] Denied access to /base/data/home/apps/s~myadmin
// W1016 16:40:58.490329 24829 fd_proxy.cc:125] Denied access to /base/data/home/apps
// W1016 16:40:58.491711 24829 fd_proxy.cc:125] Denied access to /base/data/home
// W1016 16:40:58.492526 24829 fd_proxy.cc:125] Denied access to /base/data
// W1016 16:40:58.493359 24829 fd_proxy.cc:125] Denied access to /base
// So we can shortcut this function that once we get to the root of the
// application we can complete the recursion.
// To do this, we can add a field to the structure sapi_globals, which
// is the applications base folder. Once we see that path matches this we
...
static int tsrm_realpath_r(char *path, int start, int len, int *ll, time_t *t, int use_realpath, int is_dir, int *link_is_dir TSRMLS_DC) /* {{{ */
{
	...
	// Notes(slangley): If the path segment that we have been passed here matches
	// one of the application path roots, then we will terminate the recursion as
	// the application does not have access to the higher directories in this
  // stack.
  if ((start < len) && SG(application_path_roots)) {
    int i;
    for (i=0; SG(application_path_roots)[i] != NULL; ++i) {
      if (strncmp(SG(application_path_roots)[i], path, len) == 0) {
        // Ok, the remaining path to expand matches and application root path,
        // so we can bail out. Ensure we set a null character as expected and
        // return the position
        path[len] = 0;
        return len;
      }
