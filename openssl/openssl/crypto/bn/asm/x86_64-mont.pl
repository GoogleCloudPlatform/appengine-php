...
.Lsqr4x_copy:				# copy or in-place refresh
	movdqu  ($tptr,$i),%xmm2
	movdqu	16($tptr,$i),%xmm4
	movdqu	($rptr,$i),%xmm1
	movdqu	16($rptr,$i),%xmm3
	pxor	%xmm1,%xmm2		# conditional select
	pxor	%xmm3,%xmm4
	pand	%xmm0,%xmm2
	pand	%xmm0,%xmm4
	...
	pxor	%xmm3,%xmm4
	movdqu	%xmm2,($rptr,$i)
	movdqu	%xmm4,16($rptr,$i)
	movdqa	%xmm5,64(%rsp,$i)	# zap lower half of temporary vector
	movdqa	%xmm5,80(%rsp,$i)	# zap lower half of temporary vector
	movdqa	%xmm5,($tptr,$i)	# zap upper half of temporary vector
	movdqa	%xmm5,16($tptr,$i)	# zap upper half of temporary vector
	lea	32($i),$i
	dec	$j
	jnz	.Lsqr4x_copy
___
